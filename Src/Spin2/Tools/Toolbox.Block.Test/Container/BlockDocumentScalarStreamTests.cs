...
public class BlockDocumentScalarStreamTests
{
    ...
    public void GivenSingleDocument_ShouldPass()
    {
        ...
        var doc = new BlockDocument(_owner, "schema/tenant/path".ToObjectId())
            .Add(_ownerSignature)
        ...
        BlockDocument doc2 = BlockDocument.Create(json, "schema/tenant/path".ToObjectId());
    ...
    //    BlockDocument doc2 = BlockDocument.Create(zip, "schema/tenant/path".ToObjectId());
    //    string doc2MerkleTreeValue = doc.GetMerkleTreeValue();
    //    merkleTreeValue.Should().Be(doc2MerkleTreeValue);
