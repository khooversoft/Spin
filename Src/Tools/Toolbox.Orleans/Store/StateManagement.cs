...
public class StateManagement
{
    ...
    public StateManagement(IDatalakeStore datalakeStore, string monitorPath, ILogger<StateManagement> logger)
    {
        _datalakeStore = datalakeStore.NotNull();
        _monitorPath = monitorPath.NotEmpty();
    ...
    private async Task CheckDirectoryFile(ScopeContext context)
    {
        ...
        context.LogInformation("Checking ETag of directory file={file}", _monitorPath);

        Option<DatalakePathProperties> readPathPropertyOption = await GetPathProperties(_monitorPath, context);
        ...
        if (e1 == _eTag)
        {
            context.LogInformation("Directory's file={file} ETag has not changed, ETag={etag}, {store.etag}", _monitorPath, e1, _eTag);
            return;
        }

        context.LogInformation("Directory's file={file} ETag has changed, ETag={etag}, {store.etag}", _monitorPath, e1, _eTag);
        _eTag = e1;
        _cacheDataMap.Clear();
        return;
    }
