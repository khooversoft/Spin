...
{
    public static string ScalarFile(PathDetail fileDetail) => $"{fileDetail.NotNull().PipelineName}/{fileDetail.TypeName}/{fileDetail.Key}-{fileDetail.TypeName}";

    public static string DailyListPartitioning(PathDetail fileDetail)
    {
        fileDetail.NotNull();
        DateTime now = DateTime.UtcNow;
        return $"{fileDetail.PipelineName}/{fileDetail.TypeName}/{now:yyyyMM}/{fileDetail.Key}-{now:yyyyMMdd}-{fileDetail.TypeName}";
    }

    public static string DailyListPartitionSearch(PathDetail fileDetail) => $"{fileDetail.NotNull().PipelineName}/{fileDetail.TypeName}/**/{fileDetail.Key}-*-{fileDetail.TypeName}*";
    ...
    public static string BatchPartitioning(PathDetail fileDetail)
    {
        ...
        return $"{fileDetail.PipelineName}/{fileDetail.TypeName}/{now:yyyyMM}/{fileDetail.Key}-{now:yyyyMMdd}-{randString}-{fileDetail.TypeName}";
    }
}
