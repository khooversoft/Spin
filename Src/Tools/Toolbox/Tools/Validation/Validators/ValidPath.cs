...
public class ValidPathOption<T> : ValidatorBase<T, string?>
{
    public ValidPathOption(IPropertyRule<T, string?> rule, string errorMessage)
        : base(rule, errorMessage, x => x == null || IdPatterns.IsPath(x))
    {
    }
}
...
{
    public static Rule<T, string> ValidPath<T>(this Rule<T, string> rule, string errorMessage = "valid Path is required")
    {
        rule.PropertyRule.Validators.Add(new ValidPath<T>(rule.PropertyRule, errorMessage));
        return rule;
    }

    public static Rule<T, string?> ValidPathOption<T>(this Rule<T, string?> rule, string errorMessage = "valid Path is required")
    {
