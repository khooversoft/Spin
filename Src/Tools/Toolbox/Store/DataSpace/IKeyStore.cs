...
{
    Task<Option<string>> Add(string path, DataETag data, ScopeContext context, TrxRecorder? recorder = null);
    ...
    Task<Option<string>> AcquireExclusiveLock(string path, bool breakLeaseIfExist, ScopeContext context);
    Task<Option<string>> AcquireLease(string path, TimeSpan leaseDuration, ScopeContext context);
}
