...
public class SequenceSpaceProvider : IStoreSequenceProvider
{
    ...
    public ISequenceStore<T> GetStore<T>(SpaceDefinition definition, SpaceOption<T> options)
    {
        ...
        SequenceKeySystem<T> listKeySystem = new(definition.BasePath, logSequenceNumber);

        var store = ActivatorUtilities.CreateInstance<SequenceSpace<T>>(_serviceProvider, listKeySystem, options);
