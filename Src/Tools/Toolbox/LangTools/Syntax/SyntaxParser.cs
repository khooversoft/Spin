...
public class SyntaxParser
{
    ...
    public SyntaxResponse Parse(string rawData, ILogger logger)
    {
        logger.LogDebug("Parsing rawData={rawData}", rawData);
        ...
        logger.LogDebug("Parsed data, tokens.Count={tokensCount}", tokens.Count);
        ...
        {
            status2 = ProcessRules(pContext, logger);
            if (status2.IsError()) break;
        }

        logger.LogStatus(status2, "Parsing rules exit");
        ...
        logger.LogStatus(status, "Parse completed");
    ...
    private Option ProcessRules(SyntaxParserContext pContext, ILogger logger)
    {
        logger.LogDebug("ProcessingRules: pContext={pContext}", pContext.GetDebuggerDisplay());

        foreach (var rule in _rootRules)
        {
            logger.LogDebug("[Rule] processing: pContext{pContext}", rule.GetDebuggerDisplay());

            var treeBuilder = new SyntaxTreeBuilder { MetaSyntax = rule };
            Option status = ProcessRule(pContext, rule, treeBuilder, logger);
            if (status.IsOk() && treeBuilder.Children.Count > 0) pContext.SyntaxTree.Children.Add(treeBuilder.ConvertTo());

            if (status.IsNotFound())
            {
                logger.LogDebug("[Rule] - no match - processing: pContext{pContext}", rule.GetDebuggerDisplay());
                continue;
            }
    ...
    private Option ProcessRule(SyntaxParserContext pContext, IMetaSyntax parentMetaSyntax, SyntaxTreeBuilder tree, ILogger logger)
    {
        ...
        while (stack.TryPop(out var syntax))
        {
            switch (syntax)
            {
                case TerminalSymbol terminal:
                    returnStatus = ProcessTerminal(pContext, terminal, tree, logger);
                    break;

                case VirtualTerminalSymbol virtualTerminal:
                    returnStatus = ProcessVirtualTerminal(pContext, virtualTerminal, tree, logger);
                    break;

                case ProductionRule rule:
                    var treeBuilder = new SyntaxTreeBuilder { MetaSyntax = rule };
                    returnStatus = ProcessRule(pContext, syntax, treeBuilder, logger);
                ...
                case ProductionRuleReference referenceRule:
                    ...
                    {
                        logger.LogError("ProcessRule: Reference syntax={referenceSyntax} not found", referenceRule.ReferenceSyntax);
        ...
        {
            logger.LogDebug("ProcessRule: Failed to match rule, pContext{pContext}", pContext.GetDebuggerDisplay());
            return StatusCode.NotFound;
        }

        scope.Cancel();
        logger.LogDebug("ProcessRule: Success, pContext{pContext}", pContext.GetDebuggerDisplay());
        return returnStatus;
    ...
    private Option ProcessTerminal(SyntaxParserContext pContext, TerminalSymbol terminal, SyntaxTreeBuilder tree, ILogger logger)
    {
        return ProcessTerminalValue(pContext, terminal, tree, isTokenMatch, logger);
    ...
    private Option ProcessVirtualTerminal(SyntaxParserContext pContext, VirtualTerminalSymbol terminal, SyntaxTreeBuilder tree, ILogger logger)
    {
        return ProcessTerminalValue(pContext, terminal, tree, x => x == terminal.Text, logger);
    }

    private Option ProcessTerminalValue(SyntaxParserContext pContext, IMetaSyntax terminal, SyntaxTreeBuilder tree, Func<string, bool> isTokenMatch, ILogger logger)
    {
        using var scope = pContext.PushWithScope();

        if (!pContext.TokensCursor.TryGetValue(out var current))
        {
            logger.LogDebug("ProcessTerminal: No token found, pContext{pContext}", pContext.GetDebuggerDisplay());
        ...
        switch (current)
        {
            case var v when isTokenMatch(v.Value):
                ...
                logger.LogDebug("ProcessTerminal: Add Terminal: token=[{token}], terminal=[{terminal}]", v.GetDebuggerDisplay(), terminal.GetDebuggerDisplay());
                scope.Cancel();
                return StatusCode.OK;

            default:
                logger.LogDebug("ProcessTerminal: Terminal does not match, currentToken={tokenValue}, terminal={terminal} pContext={pContext}",
