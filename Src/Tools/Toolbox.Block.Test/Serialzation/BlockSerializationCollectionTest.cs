...
public class BlockSerializationCollectionTest
{
    ...
    [Fact]
    public void WhenRecordDeleteToDocumentWithOneAdd_RoundTrip_ShouldPass()
    {
        var document = new BlockDocument();
        ...
        a = a with { StringList = a.StringList.Take(2).ToArray() };
        a = a with { LedgerItems = a.LedgerItems.Skip(1).ToArray() };
        document.Add(a);
