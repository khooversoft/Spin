...
public class AddUniqueIndexTests
{
    ...
    public async Task AddMultipleIndexes()
    {
        ...
        var e1 = await graphClient.Execute("add node key=node1 set t1=v1, t2=v2 index t1, t2 ;");
        e1.IsOk().BeTrue();
        ...
        var e2 = await graphClient.Execute("add node key=node2 set t3=v3 index t3 ;");
        e2.IsOk().BeTrue();
    ...
    public async Task AddMultipleIndexesDifferentValue()
    {
        ...
        var e2 = await graphClient.Execute("add node key=node2 set t1=v2 index t1 ;");
        e2.IsOk().BeTrue();
    ...
    public async Task UniqueIndexViolation()
    {
        ...
        var e2 = await graphClient.Execute("add node key=node2 set t1=v1 index t1 ;");
        e2.IsError().BeTrue();
    ...
    public async Task UniqueIndexViolation2()
    {
        ...
        var e2 = await graphClient.Execute("add node key=node3 set t1=v2 index t1 ;");
        e2.IsOk().BeTrue();

        var e3 = await graphClient.Execute("add node key=node2 set t1=v1 index t1 ;");
        e3.IsError().BeTrue();
