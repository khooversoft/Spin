...
public class GraphSchemaMultipleReferencesTests
{
    ...
    public async Task SetCommand()
    {
        ...
        (await testClient.ExecuteBatch(new string[] {
            "upsert node key=role:owner@domain.com;",
            "upsert node key=role:role1;",
            "upsert node key=role:role2;",
            "upsert node key=role:owner@domain.com-2;",
            "upsert node key=role:role1-2;",
            "upsert node key=role:role2-2;",
        }.Join(Environment.NewLine), NullScopeContext.Instance)).IsOk().Should().BeTrue();
        ...
        string[] expectedCmds = [
            "upsert node key=data:key1, entity { 'eyJrZXkiOiJrZXkxIiwibmFtZSI6Im5hbWUxIiwib3duZXJSZWZlcmVuY2UiOiJvd25lckBkb21haW4uY29tIiwicm9sZXMiOlt7InByaW5jaXBhbElkIjoicm9sZTEifSx7InByaW5jaXBhbElkIjoicm9sZTIifV19' };",
            "upsert edge fromKey=data:key1, toKey=role:owner@domain.com, edgeType=data-to-role;",
            "upsert edge fromKey=data:key1, toKey=role:role1, edgeType=data-to-role;",
            "upsert edge fromKey=data:key1, toKey=role:role2, edgeType=data-to-role;",
            ];
        ...
        expectedCmds = [
            "upsert node key=data:key1, entity { 'eyJrZXkiOiJrZXkxIiwibmFtZSI6Im5hbWUxIiwib3duZXJSZWZlcmVuY2UiOiJvd25lckBkb21haW4uY29tLTIiLCJyb2xlcyI6W3sicHJpbmNpcGFsSWQiOiJyb2xlMS0yIn0seyJwcmluY2lwYWxJZCI6InJvbGUyLTIifV19' };",
            "delete [fromKey=data:key1, toKey=role:owner@domain.com, edgeType=data-to-role];",
            "delete [fromKey=data:key1, toKey=role:role1, edgeType=data-to-role];",
            "delete [fromKey=data:key1, toKey=role:role2, edgeType=data-to-role];",
            "upsert edge fromKey=data:key1, toKey=role:owner@domain.com-2, edgeType=data-to-role;",
            "upsert edge fromKey=data:key1, toKey=role:role1-2, edgeType=data-to-role;",
            "upsert edge fromKey=data:key1, toKey=role:role2-2, edgeType=data-to-role;",
            ];
