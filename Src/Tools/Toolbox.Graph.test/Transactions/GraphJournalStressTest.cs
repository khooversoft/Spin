...
//            // Compare files
//            var sourceFiles = await fileStore.Search("basepath/data/**/*");
//            sourceFiles.Count.Assert(x => x > 0, "Source files should not be empty");

//            var destinationFiles = await fileStore.Search("db-recovery/data/**/*");
...
//            var sourceFilesWithFileHashes = (await fileStore.GetFileHashes(sourceFiles)).BeOk().Return();
//            var destinationFilesWithHashes = (await fileStore.GetFileHashes(destinationFiles)).BeOk().Return();
...
//            (await graphClient.ExecuteBatch(cmd)).BeOk().Return().Items.Count.Be(1);
...
//            (await graphClient.ExecuteBatch(cmd)).BeOk().Return().Items.Count.Be(2);
...
//            (await graphClient.ExecuteBatch(cmd)).BeOk().Return().Items.Count.Be(3);
...
//            var result = (await graphClient.ExecuteBatch(cmd)).BeOk().Return();
