...
public class DirectoryActor : Grain, IDirectoryActor
{
    ...
    public DirectoryActor(
        ...
        [PersistentState("default", TsConstants.DataLakeProviderName)] IPersistentState<GraphSerialization> state,
    ...
    {
        this.GetPrimaryKeyString().Assert(x => x == TsConstants.DirectoryActorKey, x => $"Actor key {x} is not {TsConstants.DirectoryActorKey}");
