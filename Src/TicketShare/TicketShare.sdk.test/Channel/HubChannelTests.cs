...
public class HubChannelTests
{
    ...
    public async Task GeneralMessageFlow()
    {
        ...
        await AddHubChannel(manager, client, _channelId, [_friend1, _friend2], context);

        string messageId1 = await SendMessage(manager, client, _friend1, "message #1", context);
        string messageId2 = await SendMessage(manager, client, _friend2, "message #2", context);
        await TestChannelMessages(manager, client, 2, context);

        string messageId3 = await SendMessage(manager, client, _principalId, "message #3", context);
        await TestChannelMessages(manager, client, 3, context);

        await SearchByUser(manager, client, _principalId, context);
        await SearchByUser(manager, client, _friend1, context);
        await SearchByUser(manager, client, _friend2, context);
    }
    ...
    private async Task AddHubChannel(HubChannelManager manager, HubChannelClient client, string channelId, IEnumerable<string> users, ScopeContext context)
    {
        ...
        var addFriend1Option = await hubContext.Principals.Set(_friend1, ChannelRole.Contributor, context);
        addFriend1Option.IsOk().Should().BeTrue();

        var addFriend2Option = await hubContext.Principals.Set(_friend2, ChannelRole.Contributor, context);
        ...
        var result = await hubContext.Get(context);
