...
public static class TeamMasterList
{
    ...
    private static IReadOnlyList<TeamDetail>? _details;

    public static IReadOnlyList<TeamDetail> GetDetails() => _details ??= InternalGetDetails();

    private static IReadOnlyList<TeamDetail> InternalGetDetails()
    {
        var details = _files
            .Select(x => (league: getleague(x), resourceId: $"TicketApi.sdk.MasterList.{x}"))
        ...
        details.ForEach(x => x.Validate().ThrowOnError());
        return details;
